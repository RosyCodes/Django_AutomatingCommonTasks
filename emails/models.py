...
class Email(models.Model):
    ...
    def open_rate(self):
        ...
        # extracts records with non-empty opened_at field
        opened_count = EmailTracking.objects.filter(
            email=self, opened_at__isnull=False).count()
    ...
    def click_rate(self):
        ...
        if opened_count > 0:
            # Formula
            clicked_count = EmailTracking.objects.filter(
                email=self, clicked_at__isnull=False).count()
            click_rate = (clicked_count/opened_count) * \
...
class EmailTracking(models.Model):
    email = models.ForeignKey(
        Email, on_delete=models.CASCADE, null=True, blank=True)
